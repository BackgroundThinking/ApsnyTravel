...
describe('API Service (Mock Mode)', () => {
  ...
  it('fetchTourBySlug should return the correct tour', async () => {
    ...
    if (!activeTour)
      throw new Error('No active tours in constants to test with');
  ...
  it('fetchTourBySlug should throw 404 for unknown slug', async () => {
    await expect(fetchTourBySlug('non-existent-slug-123')).rejects.toThrow(
      'Тур не найден',
    );
  ...
  it('fetchTourBySlug should throw 404 for inactive tour slug', async () => {
    ...
    if (inactiveTour) {
      await expect(fetchTourBySlug(inactiveTour.slug)).rejects.toThrow(
        'Тур не найден',
      );
    }
  });
